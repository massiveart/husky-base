    ...
    return {
        ...
        renderNavigationItems: function(data) {
            ...
            this.sandbox.util.foreach(data.items, function(section) {
                ...
                this.sandbox.util.foreach(section.items, function(item) {
                    // create item
                    $elem = this.sandbox.dom.createElement(this.sandbox.util.template(templates.mainItem, {
                        item: item,
        ...
        renderSubNavigationItems: function(data, $parentList) {
            ...
            this.sandbox.util.foreach(data.items, function(item) {
                ...
                this.items.push(item);
                if (item.items && item.items.length > 0) {
                    elem = this.sandbox.dom.createElement(this.sandbox.util.template(templates.subToggleItem, {item: item, translate: this.sandbox.translate}));
                    this.renderSubNavigationItems(item, this.sandbox.dom.find('div', elem));
                } else {
                    elem = this.sandbox.dom.createElement(this.sandbox.util.template(templates.subItem, {item: item, translate: this.sandbox.translate}));
                }
